...
parser.add_argument("-superclasses", type=int, default=15, help="number of superclasses")  #20
...
checkpoint_path = os.path.join(config.CHECKPOINT_PATH,
                               ...
                               "{net}-{dataset}-{classes}".format(net=args.net,
                                                                  dataset=args.dataset,classes=args.superclasses),
                               "{task}".format(task="unlearning"),
                               "{unlearning_method}-{para1}-{para2}".format(unlearning_method=args.method, para1=args.para1,
                                                                            para2=args.para2))
...
#     classwise_train, classwise_test, args.subclasses, args.forget_class
# )
...
#
# ood_subclass = []
# for key in config.ood_classes:
#     ood_subclass.extend(coarse_map[key])
...
kwargs = {
    ...
    "forget_subclass": int(args.forget_class),
    "forget_class": forget_superclass,
...
# Time the method
import time
print("forget_sub_class_main_cifar15")
# executes the method passed via args
(d_t, d_f, d_r, mia), time_elapsed = getattr(forget_full_class_strategies, args.method)(**kwargs) #forget_full_class_strategies
