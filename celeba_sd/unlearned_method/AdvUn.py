from SD.ldm.models.diffusion.ddim import DDIMSampler
from celeba_sd.advun_utils.util import *
from celeba_sd.advun_utils.attack_util import *
from celeba_sd.advun_utils.text_encoder import CustomTextEncoder
from celeba_sd.advun_utils.get_loss import *
from celeba_sd.advun_utils.prompt_dataset import *
...
def AdvUnlearn(pipeline, forget_prompt, forget_dataloader, tokenizer, attack_method, norm_layer, component,
               start_guidance, negative_guidance, iterations, save_interval, lr, config_path,
               ckpt_path, diffusers_config_path, output_dir, devices, seperator=None, image_size=512,
    ...
    # ===== Concept =====
    train_method = 'text_encoder_layer0'

    # ====== Stage 0: PROMPT CLEANING ======
    word_print = forget_prompt.replace(' ','')
    ...
    if 'text_encoder' in train_method:
        ...
        parameters = param_choices(model=unet, train_method=train_method, component=component, final_layer_norm=norm_layer)
    ...
    for i in pbar:
            ...
            if i >= warmup_iter:
                ...
                if attack_round == 0:
                    if attack_embd_type == 'word_embd':
                        adv_word_embd, adv_input_ids = soft_prompt_attack(global_step, word, pipeline, tokenizer, custom_text_encoder,
                                                                          sampler, emb_0, emb_p,  start_guidance,  devices,
                                                                          ddim_steps, ddim_eta, image_size, criteria, adv_prompt_num,
                                                                          all_embeddings, attack_round, attack_type,  attack_embd_type,
                                                                          attack_step, attack_lr, attack_init, None, attack_method)
                    elif attack_embd_type == 'condition_embd':
                        adv_condition_embd, adv_input_ids = soft_prompt_attack(global_step, pipeline, tokenizer, custom_text_encoder,
                                                                               sampler, emb_0, emb_p, start_guidance,  devices,
                                                                               ddim_steps, ddim_eta, image_size, criteria, adv_prompt_num,
                                                                               all_embeddings, attack_round, attack_type, attack_embd_type,
                                                                               attack_step, attack_lr, attack_init, None, attack_method)
                    ...
                    if attack_embd_type == 'word_embd':
                        adv_word_embd, adv_input_ids = soft_prompt_attack(global_step, word, pipeline, tokenizer,
                                                                          custom_text_encoder, sampler, emb_0, emb_p,  start_guidance,  devices,
                                                                          ddim_steps, ddim_eta, image_size, criteria, adv_prompt_num, all_embeddings,
                                                                          attack_round, attack_type,  attack_embd_type, attack_step, attack_lr, attack_init,
                                                                          adv_word_embd, attack_method)
                    elif attack_embd_type == 'condition_embd':
                        adv_condition_embd, adv_input_ids = soft_prompt_attack(global_step, word, pipeline, tokenizer, custom_text_encoder, sampler,
                                                                               emb_0, emb_p, start_guidance,  devices, ddim_steps, ddim_eta, image_size, criteria,
                                                                               adv_prompt_num, all_embeddings, attack_round, attack_type, attack_embd_type, attack_step,
                                                                               attack_lr, attack_init, adv_condition_embd, attack_method)

                global_step += attack_step
                attack_round += 1

        # Set model/TextEnocder to train or eval mode
        if 'text_encoder' in train_method: #tune the text_encoder
        ...
        if i < warmup_iter:
            ...
            loss = get_train_loss_retain(retain_batch, retain_train, retain_loss_w, model, model_orig,
                                         custom_text_encoder, sampler, emb_0, emb_p, retain_emb_p, emb_n, retain_emb_n,
                                         start_guidance, negative_guidance, devices, ddim_steps, ddim_eta, image_size,
                                         criteria, input_ids, attack_embd_type)
        else:
            if attack_embd_type == 'word_embd':
                loss = get_train_loss_retain(retain_batch, retain_train, retain_loss_w, model, model_orig, custom_text_encoder,
                                             sampler, emb_0, emb_p, retain_emb_p, None, retain_emb_n, start_guidance,
                                             negative_guidance, devices, ddim_steps, ddim_eta, image_size, criteria, adv_input_ids,
                                             attack_embd_type, adv_word_embd)
            elif attack_embd_type == 'condition_embd':
                loss = get_train_loss_retain(retain_batch, retain_train, retain_loss_w, model, model_orig, custom_text_encoder, sampler,
                                             emb_0, emb_p, retain_emb_p, None, retain_emb_n, start_guidance, negative_guidance,
                                             devices, ddim_steps, ddim_eta, image_size, criteria, adv_input_ids, attack_embd_type, adv_condition_embd)
...
if __name__ == '__main__':
    ...
    AdvUnlearn(prompt=prompt, dataset_retain=dataset_retain, retain_batch= retain_batch,
               retain_train=retain_train, retain_step=retain_step, retain_loss_w=retain_loss_w,
               attack_method = attack_method, train_method=train_method, norm_layer=norm_layer,
               component=component, start_guidance=start_guidance, negative_guidance=negative_guidance,
               iterations=iterations, save_interval=save_interval, lr=lr, config_path=config_path, ckpt_path=ckpt_path,
               diffusers_config_path=diffusers_config_path, output_dir= output_dir, devices=devices, seperator=seperator,
               image_size=image_size, ddim_steps=ddim_steps, adv_prompt_num=adv_prompt_num, attack_embd_type=attack_embd_type,
               attack_type=attack_type, attack_init=attack_init, warmup_iter=warmup_iter, attack_step=attack_step, attack_lr=attack_lr,
               adv_prompt_update_step=adv_prompt_update_step)