...
class TextGrad(Attacker):
    ...
    def run(self, task, logger, instance_data_dir, cand_idx):

        for handler in logging.root.handlers[:]:
            logging.root.removeHandler(handler)
        timestamp = time.strftime("%m%d_%H%M%S")
        log_path = f'{self.cache_path}/log'
        os.makedirs(log_path, exist_ok=True)
        log_file_path = f'{log_path}/usim_cand{cand_idx}_{timestamp}.log'
        ...
        results = task.eval(viusalize_prompt_id,prompt, guidance_scale=guidance)
        results['prompt'] = prompt
        # logger.save_img('orig', results.pop('image'))
        save_image = results['image']
        save_image.save(self.cache_path + f"/{prompt}.png")
        logger.log(results)
                ...
                for t in task.sampled_t:
                    ...
                    for i in range(self.iteration):
                        ...
                        input_arguments = {"x0":x0,"t":t,"input_ids":input_ids,"input_embeddings":adv_input_embeddings,
                                           'orig_input_ids':viusalize_prompt_id,"orig_input_embeddings":visualize_embedding,
                    ...
                    for i in range(figure_num):
                        ...
                        save_image = results['image']
                        save_image.save(self.cache_path+f"/{t}_{new_visualize_prompt}_{i}.png")
                        print("save_image", self.cache_path+f"/{t}_{new_visualize_prompt}_{i}.png")
                        logger.log(results)
                        # calculate ISM for forget persons
                    image_name_list = [f'{t}_{new_visualize_prompt}_{i}.png' for i in range(figure_num)]
                    print("instance_data_dir", instance_data_dir)
                    ism_inferred = matching_score_genimage_id(self.cache_path, [instance_data_dir], image_name_list)
                    print("[inferred] ISM and FDR are {}".format(ism_inferred))
                    logging.info(f'Step: {t}\t Adv_Prompt: {new_visualize_prompt}\t Infer-ISM: {ism_inferred}')
                    if results.get('success') is not None and results['success']:
                        break
            else:
