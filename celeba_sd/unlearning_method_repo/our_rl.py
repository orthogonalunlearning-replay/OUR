...
def our_random_label(pipeline, forget_dataloader, retain_dataloader, lr, epochs,
                 noise_scheduler, tokenizer, device, with_prior_preservation=False, mask=None, saved_model_path='salun'):
    unet = pipeline.unet
    pipeline.safety_checker = None
    unet.train()
    text_encoder = pipeline.text_encoder #makes forget harder
    text_encoder.train()
    ...
    for epoch in range(epochs):
        for step, (forget_batch, retain_batch) in enumerate(tqdm(zip(forget_dataloader, retain_dataloader), desc=f"Epoch {epoch}")):
            ...
            noise_pesudo = torch.randn_like(latents2)
            forget_loss =  F.mse_loss(model_pred_forget, noise_pesudo.float()) + F.mse_loss(model_pred_forget, model_pred_pesudo)
