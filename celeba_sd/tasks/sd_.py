...
class SDTask:
    def __init__(
        ...
        self.unet_sd = UNet2DConditionModel.from_pretrained(model_name_or_path, subfolder="unet").to(self.device)
    ...
    def eval(self,input_ids,prompt,seed=0,batch_size=1,height=512,width=512,guidance_scale=7.5):
        ...
        image = self.sampling(input_ids,text_embeddings,batch_size=batch_size,
                              seed=seed,height=height,width=width,num_inference_steps=self.sampling_step_num,guidance_scale=guidance_scale)
        ...
        results['object']=0#, logits = object_eval(self.classifier,results['image'], processor=self.processor, device=self.device)
        results['score']=0# = logits[self.object_labels[self.object_list.index(self.concept)]].item()
        results['success']=0# = results['object'] == self.object_labels[self.object_list.index(self.concept)]
        # os.remove(img_tmp_path)
        return results

def get(**kwargs):
    return SDTask(**kwargs)