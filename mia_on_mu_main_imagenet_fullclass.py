...
parser.add_argument(
        ...
        default='Cifar20',
        nargs="?",
        # choices=["GTSRB", "Cifar19", "Cifar10", "Cifar20", "Cifar100", "PinsFaceRecognition"],
...
if __name__ == '__main__':
    ...
    classwise_train, classwise_test = get_classwise_ds_gtsrb(trainset, num_classes=100), \
        get_classwise_ds_gtsrb(validset, num_classes=100)

    (retain_train, retain_valid) = build_retain_sets_in_unlearning_gtsrb(classwise_train, classwise_test, int(args.classes),
                                                                         int(args.forget_class),
                                                     config.ood_classes)

    forget_train, forget_valid = classwise_train[int(args.forget_class)], classwise_test[int(args.forget_class)]

    # for idx, (img, label) in enumerate(forget_train):
    #     tensor = (img * 255).to(torch.uint8)
    #     image = TF.to_pil_image(tensor)
    #     image.save(f"forget_images_idx{idx}.png")
    #     if idx > 5:
    #         break
    ...
    print("len_forget_train", len(forget_train))
    ...
    for cls in config.ood_classes:
        ...
        for img, label in classwise_test[cls]:
            ood_valid_ds[cls].append((img, int(args.forget_class)))  # 分布外的数据默认为遗忘类

        for img, label in classwise_train[cls]:
            ood_train_ds[cls].append((img, int(args.forget_class)))  # 分布外的数据默认为遗忘类

        # for idx, (img, label) in enumerate(classwise_train[cls]):
        #     tensor = (img * 255).to(torch.uint8)
        #     image = TF.to_pil_image(tensor)
        #     image.save(f"ood_images_cl{cls}_idx{idx}.png")
        #     if idx > 5:
        #         break
